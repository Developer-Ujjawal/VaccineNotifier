...
function createTemplate(slotDetails, date){
    let message = `Hello, 
    <br/>
    Yepieeee!!! Vaccine is available on <strong> ${date} </strong> in the below centers: 
    <br/><br/>
    `
    ...
    for(const slot of slotDetails){
        ...
        if(slot && slot.slots) {
            ...
            const noSlotMessage = "No slots Available!"
            slotBody = `${slotBody} ${noSlotMessage} <br/>`
        }
...
exports.sendEmail = function (email, subjectLine, slotDetails, date, callback) {
    ...
    let options = {
        from: String('Vaccine Search ' + process.env.EMAIL),
        to: email,
        subject: subjectLine,
        html: message
    };
